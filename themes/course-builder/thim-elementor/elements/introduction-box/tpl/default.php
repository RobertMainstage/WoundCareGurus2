...
$box          = array_filter( (array) $setting['box'] );
$number_items = count( $box );

$el_id    = $setting['el_id'] ? ' id="' . $setting['el_id'] . '"' : '';
$el_class = $setting['el_class'] ? ' ' . $setting['el_class'] : '';
...
if ( !empty( $box ) ) {
	...
	<div class="thim-sc-intro-box<?php echo esc_attr( $el_class ); ?>"<?php echo esc_attr( $el_id ); ?>>
		...
		<?php
		if ( $setting['bg-image'] ) {
			$attachment_bg_id = intval( $setting['bg-image']['id'] );
								...
								<div class="box-wrapper">
									<?php if ( !empty( $content['image'] ) ): ?>
										<div class="single-image">
											<?php
											$attachment_id = intval( $content['image']['id'] );
									...
									<div class="content-wrapper">
										...
										if ( !empty( $content['read_more'] ) ) {
											...
											echo '<a href="' . ( $link_detail['url'] ) . '"' . ( $link_output ) . ' class="more-detail">' . esc_html( $content['read_more_title'] ) . '</a>';
										}
										?>
									</div>
								</div>
