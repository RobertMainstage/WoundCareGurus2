...
$course_cat = $setting['course_cat'];
$course_list = $setting['course_list'];

$col_class = 12 / intval($setting['cols']);

$course_cat_query_args = array(
	'post_type'      => LP_COURSE_CPT,
	'post_status'    => 'publish',
	'posts_per_page' => $setting['course_limit'],
...
if ( $setting['course_featured'] == 'yes' ) {
...
<?php if ( $title || $course_cat_query->have_posts() ) { ?>
	...
	<div class="thim-course-block-3 <?php echo esc_attr($title_center); ?>  <?php if ( $setting['filter'] !== 'yes') { echo 'has-filter';}?>
	...
	<?php if ( $course_cat_query->have_posts() ) {
		...
		$html_main_content .= '<div class="course-item col-sm-'. $col_class .' ' . esc_attr( $course_label ) . ' '. esc_attr($free) .' ' . esc_attr( $price_class ) . ' ' . esc_attr( $new_course_label ) . ' ' . esc_attr( $class_categories ).' ">';
        if ( $setting['cols'] === '3' ) {
        ...
        <?php if ( empty( $course_cat ) ) { ?>
            ...
            <?php } ?>
         <?php } else {
            $course_cat_id = $setting['course_cat'];
            ?>
            <div class="masonry-filter">
                <?php if ($setting['filter'] !== 'yes'){ ?>
                    <div class="masonry-filter">
