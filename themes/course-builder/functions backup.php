...
function thim_scripts() {
	...
	if ( ( is_archive() || 'post' == get_post_type() ) && ( 'loadmore' == get_theme_mod( 'blog_archive_nav_style', 'pagination' ) || ( ( isset( $_GET['pagination'] ) ? $_GET['pagination'] : '' ) === 'loadmore' ) ) ) {
		wp_enqueue_script( 'thim-loadmore', THIM_URI . 'assets/js/libs/thim-loadmore.js', array(
			'jquery',
			'thim-change-layout'
		), '', true );
...
/**
 * Custom functions - ROBERT LANDMARK 3 - COMMENT THIS OUT
 */
...
if ( defined( 'THIM_CB_PATH' ) ) {
	$data = get_plugin_data( THIM_CB_PATH . "/thim-course-builder.php", true, false );
}
if ( thim_plugin_active( 'elementor' ) && $data['Version'] < '3.1.7' ) {
