        ...
        foreach ($columns as $column) {
            if (isset($column->Key)) {
                ...
                foreach ($cresults as $cresult) {
                    ...
                    } else if (fra_is_JSON($cresult->$field)) {
                        $jsondata = @json_decode($cresult->$field);
                        if (is_array($jsondata)) {
                            $jsondata = array_value_replace($jsondata, $findstr, $replacestr);
                            $updatesql = $wpdb->prepare("UPDATE $table SET `$field` = %s WHERE `$primary_column`='$primary_id'", array(json_encode($jsondata)));
                            $wpdb->get_results($updatesql);
                        }
                    } else {
